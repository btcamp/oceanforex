...
if ($mt4Result ["CreateAccountResult"] ["ErrorCode"] == - 1) {
	header ( "location: /account/registermsg.php?success=false&returnurl=/open_account/open_realaccount/" );
	exit ();
} else {
	...
	if ($result == 1) {
		...
		if (! move_uploaded_file ( $filename, $destination )) {
			...
			// print_r ( $content ); print_r ( $mailmt4Result ); print_r ( $mailclient->getError () );
			...
			// print_r ( $argself ["body"] ); print_r ( $mailmt4Result ); print_r ( $mailclient->getError () );
