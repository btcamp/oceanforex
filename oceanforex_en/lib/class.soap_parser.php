	...
	function start_element($parser, $name, $attrs) {
		...
		foreach ( $attrs as $key => $value ) {
			...
			if ($key_prefix == 'xmlns') {
				...
				/*
				 * do arrayType ereg here [1] arrayTypeValue ::= atype asize [2] atype ::= QName rank* [3] rank ::= '[' (',')* ']' [4] asize ::= '[' length~ ']' [5] length ::= nextDimension* Digit+ [6] nextDimension ::= Digit+ ','
				 */
	...
	function end_element($parser, $name) {
		...
		if (isset ( $this->body_position ) && $pos > $this->body_position) {
			...
			if (isset ( $this->message [$pos] ['attrs'] ['href'] )) {
				...
				/*
				 * add value to parent's result, if parent is struct/array $parent = $this->message[$pos]['parent']; if($this->message[$parent]['type'] != 'map'){ if(strtolower($this->message[$parent]['type']) == 'array'){ $this->message[$parent]['result'][] = $this->message[$pos]['result']; } else { $this->message[$parent]['result'][$this->message[$pos]['name']] = $this->message[$pos]['result']; } }
				 */
			}
		}
